...
func (suite *S3TestSuite) TestListByPrefix() {
	...
	router.GET("/*path", middleware.TokenMiddleware(), Download)
	router.ServeHTTP(w, httptest.NewRequest("GET", "/dataset1?prefix=test", nil))

	response := w.Result()
	defer response.Body.Close()

	assert.Equal(suite.T(), http.StatusNotImplemented, response.StatusCode, "Request should return 501")
}
